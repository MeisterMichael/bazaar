module BazaarWeb
	...
	class YourOrdersController < YourController
		...
		def index
			...
			@orders = BazaarCore::Order.where( user: current_user ).order( created_at: :desc ).page(params[:page]).per(5)
		end

		def show
			@order = BazaarCore::Order.where( user: current_user ).find_by( code: params[:id] )
