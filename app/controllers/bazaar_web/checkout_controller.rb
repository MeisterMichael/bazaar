		...
		def create
			...
			if ( @order.pre_order? && @order.payment_method_captured? ) || ( @order.active? && @order.paid? )
				...
				begin
					...
					@thank_you_url = bazaar_web.thank_you_order_path( @order.code, format: :html, t: @expiration.to_i, d: Rails.application.message_verifier('order.id').generate( code: @order.code, id: @order.id, expiration: @expiration ), from: 'checkout', funnel: params[:funnel].to_s.gsub(/[^a-zA-Z0-9\-]/,'') )
