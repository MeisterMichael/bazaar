	...
	class SubscriptionAdminController < Bazaar::EcomAdminController
		...
		def update
			...
			if @subscription.save
				log_event( { name: 'cancel_subscription', user: current_user, category: 'ecom', on: @subscription, content: "canceled #{@subscription.user}'s subscription." } ) if @subscription.saved_changes[:status].present? && @subscription.canceled?
