module Bazaar
	...
	class OrderService < ::ApplicationService
		...
		def process_purchase_success( order, args = {} )
			...
			@subscription_service.subscribe_ordered_plans( order, payment_profile_expires_at: payment_profile_expires_at ) if @subscription_service.present? && (order.active? || order.review?)
