		...
		class AmazonPayTransactionService < Bazaar::TransactionService
			...
			def process( order, args = {} )
				...
				if order.parent.is_a?( Bazaar::Subscription )
					...
					plan_order_offer = order.order_offers.select{ |order_offer| order_offer.offer.recurring? && order_offer.subscription_interval == 1 }.first

					unless plan_order_offer.present?
					...
					subscription = plan_order_offer.subscription ||= Bazaar::Subscription.create( status: 'trash', user: order.user, subscription_plan: plan_order_offer.offer.subscription_plan, offer: plan_order_offer.offer, shipping_address: order.shipping_address, billing_address: order.billing_address )
