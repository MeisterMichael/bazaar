...
module Bazaar
	...
	class DiscountService < ::ApplicationService
		...
		def get_order_discount_errors( order, discount, args = {} )
			...
			error_messages << 'The selected discount\'s usage limit has been exhausted' if discount.limit_global.present? && OrderItem.where( item: discount ).joins(:order).merge( Order.positive_status ).count >= discount.limit_global
