...
describe "TaxJarTaxService" do
	...
	let(:address3) { SwellEcom::GeoAddress.new( first_name: 'Michael', last_name: (0...8).map { (65 + rand(26)).chr }.join, zip: '20317', phone: "1#{(0...10).map { (rand(8)+1).to_s }.join}", city: 'Washington', geo_country: SwellEcom::GeoCountry.new( name: 'United States', abbrev: 'US' ), geo_state: SwellEcom::GeoState.new( name: 'he District of Columbia', abbrev: 'DC' ) ) }
	...
	it "should calculate taxes for tennessee" do
		...
		expect( tax_order_items.count ).to eq 1
		expect( tax_order_items.collect(&:subtotal).sum ).to eq 9
