...
RSpec.describe Bazaar::CheckoutController, :type => :controller do
	render_views
	routes { Bazaar::Engine.routes }

	let(:new_trial_subscription_plan) {
		Bazaar::SubscriptionPlan.create( slug: "subscription-plan-1-#{(0...20).map { (65 + rand(26)).chr }.join}", title: "Subscription w/ Trial", trial_price: 99, trial_max_intervals: 1, price: 12900, billing_interval_unit: 'weeks', billing_interval_value: 4, trial_interval_unit: 'days', trial_interval_value: 7 )
	}
	let(:new_trial_subscription_plan2) {
		Bazaar::SubscriptionPlan.create( slug: "subscription-plan-2-#{(0...20).map { (65 + rand(26)).chr }.join}", title: "Subscription w/ Trial 199", trial_price: 199, trial_max_intervals: 1, price: 14900, billing_interval_unit: 'weeks', billing_interval_value: 2, trial_interval_unit: 'days', trial_interval_value: 3 )
	}
	let(:new_pre_order_product) {
		Bazaar::Product.create( slug: "pre-order-product-#{(0...20).map { (65 + rand(26)).chr }.join}", title: "Pre Order", price: 13900, availability: 'pre_order' )
	}

	let(:new_active_product) {
		Bazaar::Product.create( slug: "pre-order-product-#{(0...20).map { (65 + rand(26)).chr }.join}", title: "Pre Order", price: 15900, availability: 'open_availability' )
	}
	...
	describe "POST create" do
		...
		it "subscription - success" do
			...
			new_cart = Bazaar::Cart.find( cart.id )
			expect( new_cart.present? ).to eq(true)
			expect( new_cart.status ).to eq('success')

			order = Bazaar::CheckoutOrder.find_by( code: body_json['order_code'] )
			expect(order.class).to eq( Bazaar::CheckoutOrder )
			...
			expect( subscription.class ).to eq( Bazaar::Subscription )
